...
#include "time_r.h"
 ...
 time_r::time_r():day(0), hour(0), minut(0), second(0), visable_hour(true), visable_minut(true),
				   visable_second(true), format_hour_12(false), forward_b(true), flag_Activity(true)
				   {
 ...
 string time_r::get_time()
{
 ...
 string hour_s;
	if (this->hour<10)
	{
	 hour_s = string("0")+std::to_string(this->hour);
	}
	else
	{
	 hour_s = std::to_string(this->hour);
	}

 result = hour_s+":"+create_null_text(this->minut)+
...
bool time_r::get_flag_Activity()
{
  return flag_Activity;
};

void time_r::set_flag_Activity(const bool& value)
{
   this->flag_Activity = value;
};
